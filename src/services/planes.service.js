...
const getPlanesByIdCliente = (idCliente, state, fechaDesde) => {
    let params = {};
    if (state !== undefined) {
        params["planesFilter"] = state;
    }
    if (fechaDesde !== undefined && fechaDesde !== null) {
        params["fechaDesde"] = `${fechaDesde.toISOString().substring(0, 10)}`;
    }
    const options = {
        headers: {
    ...
    return axios
        .get(API_URL + API_PATH_CLIENTE + `/${idCliente}` + API_PATH_PLANES, options)
        .then((response) => {
...
const getPlanById = (idPlan) => {
    ...
    return axios
        .get(API_URL + API_PATH_PLANES + `/${idPlan}`, options)
        .then((response) => {
            return response.data;
        })
        .catch((error) => {
...
const postPlan = (plan, idCliente) => {
    return axios
        .post(API_URL + API_PATH_CLIENTE + `/${idCliente}` + API_PATH_PLANES, plan, {
            headers: {
...
const putPlan = (plan, idCliente, idPlan) => {
    return axios
        .put(API_URL + API_PATH_CLIENTE + `/${idCliente}` + API_PATH_PLANES + `/${idPlan}`, plan, {
            headers: {
                Authorization: `Bearer ${authService.getStoredSession().access_token}`,
            },
        })
...
const deletePlanById = (idCliente, idPlan) => {
    return axios
        .delete(API_URL + API_PATH_CLIENTE + `/${idCliente}` + API_PATH_PLANES + `/${idPlan}`, {
            headers: {
                Authorization: `Bearer ${authService.getStoredSession().access_token}`,
            },
        })
...
const getPlaneSummaryByIdCliente = (idCliente) => {
    ...
    return axios
        .get(API_URL + API_PATH_CLIENTE + `/${idCliente}` + API_PATH_PLANES_RESUMEN, options)
        .then((response) => {
            return response.data;
        })
        .catch((error) => {
